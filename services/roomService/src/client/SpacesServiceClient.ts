...
export default class SpacesServiceClient {
  ...
  // TODO: Need to actually make the Client send the correct data
  async createSpace(requestData: SpaceCreateRequest): Promise<SpaceCreateResponse> {
    const responseWrapper = await this._axios.post<ResponseEnvelope<SpaceCreateResponse>>('/spaces', requestData);
    return SpacesServiceClient.unwrapOrThrowError(responseWrapper);
  }

  async updateTown(requestData: SpaceUpdateRequest): Promise<void> {
  ...
  async deleteTown(requestData: SpaceDisbandRequest): Promise<void> {
  ...
  async listTowns(): Promise<SpaceListResponse> {
    const responseWrapper = await this._axios.get<ResponseEnvelope<SpaceListResponse>>('/towns');
    return SpacesServiceClient.unwrapOrThrowError(responseWrapper);
  }

  async joinTown(requestData: SpaceJoinRequest): Promise<SpaceJoinResponse> {
